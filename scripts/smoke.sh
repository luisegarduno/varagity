...
echo "[1/11] Service health states"
...
# kill switch, and `documents` is a (possibly empty) status map — never a
# 500 and never a lie about an unbuilt graph.
graph_json=$(curl -fsS "http://localhost:${API_PORT:-8000}/api/graph/status") \
    || fail "api /api/graph/status not reachable"
echo "$graph_json" | python3 -c '
import json, sys
status = json.load(sys.stdin)
assert isinstance(status["enabled"], bool), status
assert isinstance(status["building"], bool), status
documents = status["documents"]
assert isinstance(documents, dict), status
assert all(isinstance(count, int) for count in documents.values()), status
' || fail "/api/graph/status is malformed: ${graph_json}"
graph_enabled=$(echo "$graph_json" | python3 -c 'import json,sys; print(json.load(sys.stdin)["enabled"])')
pass "/api/graph/status well-formed (GRAPH_ENABLED=${graph_enabled})"
# The corpus directory is a bind mount the api container must be able to
# read — an unmounted/unreadable ./graph-docs would 500 here, not list empty.
curl -fsS "http://localhost:${API_PORT:-8000}/api/graph/documents" >/dev/null \
    || fail "api /api/graph/documents not reachable (is ./graph-docs mounted?)"
pass "/api/graph/documents lists the corpus directory"
