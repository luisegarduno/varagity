...
The graph half (spec_graphrag §12) is self-contained by comparison: the
fixture builder writes a synthetic iMessage ``chat.db`` and the graph
golden set scores engine *answers* by fact and provenance, so it needs no
stores at all — graph engines self-store in their own working directories.
